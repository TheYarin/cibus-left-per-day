...
async function getCibusLoginCookies(username, company, password) {
    ...
    function parseCookies(response) { // Shamelessly copied from https://stackoverflow.com/a/55680330/7009364
...
async function getNumberOfWorkDaysLeft(cibusLoginCookies) {
    ...
    async function wasCibusAlreadyUsedToday(cibusLoginCookies) {
        ...
        const lastTransactionDateString = document.querySelector("#ctl00_ctl01 > div > div.new-content-wrap > table > tbody > tr:nth-child(2) > td:nth-child(2)")?.innerHTML;
    ...
    async function getHolidaysOfMonth(month, year) {
        ...
        const response = await fetch(`https://www.hebcal.com/hebcal?v=1&cfg=json&maj=on&min=off&mod=on&nx=off&year=${year}&month=${month}&ss=off&mf=off&c=off&geo=none&s=off`)
...
const { username, company, password, watch: shouldWatch } = yargs(process.argv.slice(2))
    .command("$0 <username> <company> <password>")
    .option("watch", { alias: ["w", "widget"], type: 'boolean', description: "Run this every few minutes. This option is useful if you want to leave the terminal window open and have it auto-refresh." })
...
while (true) {
    try {
        ...
        if (numberOfWorkDaysLeft === 0) {
            console.log(`The month has ended!you have ${totalAmountLeftInCibus ? numberToMoneyString(totalAmountLeftInCibus) : "no money"} left.`);
        }
        else {
